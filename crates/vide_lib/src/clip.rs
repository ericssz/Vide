...
use crate::{
  api::transform::{Transform, OPENGL_TO_WGPU_MATRIX},
  effect::{EffectData, EffectRegistrationPacket, RegisteredEffectData},
  render::{RenderEvent, Time},
};
...
impl<'a> Clip<'a> {
  ...
  fn in_time_frame(&self, frame: u64) -> bool {
    self.start.map(|s| s <= frame).unwrap_or(true) && self.end.map(|e| e > frame).unwrap_or(true)
  }
  ...
  pub fn effect<E: 'static + RegisteredEffectData>(&mut self, effect: E) -> &mut Clip<'a> {
    if unsafe { !E::is_registered() } {
      self
        .effect_registration_packets
        .as_mut()
        .unwrap()
        .push(EffectRegistrationPacket {
          id: unsafe { E::get_id() },
  ...
  pub(crate) fn get_registration_packets(&mut self) -> Vec<EffectRegistrationPacket> {
    ...
    packets.extend(
      self
        .children
        .iter_mut()
        .flat_map(|child| child.get_registration_packets()),
    );
    packets
  }
  ...
  pub(crate) fn render(
    ...
    for clip in self.children.iter_mut() {
      if clip.in_time_frame(time.clip_frame) {
        ...
        events.extend(clip.render(
          time.derive_clip(clip_frame, clip_time, clip_progress),
          clip.end(clip_end),
          matrix,
        ));
      }
