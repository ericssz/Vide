...
pub enum ShaderValue {
  ...
  Sint8x4([i8; 4]),
  /// Two unsigned bytes (u8). [0, 255] converted to float [0, 1] `vec2` in
  /// shaders.
  Unorm8x2([u8; 2]),
  /// Four unsigned bytes (u8). [0, 255] converted to float [0, 1] `vec4` in
  /// shaders.
  Unorm8x4([u8; 4]),
  /// Two signed bytes (i8). [-127, 127] converted to float [-1, 1] `vec2` in
  /// shaders.
  Snorm8x2([i8; 2]),
  /// Four signed bytes (i8). [-127, 127] converted to float [-1, 1] `vec4` in
  /// shaders.
  Snorm8x4([i8; 4]),
  ...
  Sint16x4([i16; 4]),
  /// Two unsigned shorts (u16). [0, 65535] converted to float [0, 1] `vec2` in
  /// shaders.
  Unorm16x2([u16; 2]),
  /// Four unsigned shorts (u16). [0, 65535] converted to float [0, 1] `vec4` in
  /// shaders.
  Unorm16x4([u16; 4]),
  /// Two signed shorts (i16). [-32767, 32767] converted to float [-1, 1] `vec2`
  /// in shaders.
  Snorm16x2([i16; 2]),
  /// Four signed shorts (i16). [-32767, 32767] converted to float [-1, 1]
  /// `vec4` in shaders.
  Snorm16x4([i16; 4]),
  ...
  Sint32x4([i32; 4]),
  // /// One double-precision float (f64). `double` in shaders. Requires VERTEX_ATTRIBUTE_64BIT
  // features. Float64(f64),
  // /// Two double-precision floats (f64). `dvec2` in shaders. Requires VERTEX_ATTRIBUTE_64BIT
  // features. Float64x2([f64; 2]),
  // /// Three double-precision floats (f64). `dvec3` in shaders. Requires VERTEX_ATTRIBUTE_64BIT
  // features. Float64x3([f64; 3]),
  // /// Four double-precision floats (f64). `dvec4` in shaders. Requires VERTEX_ATTRIBUTE_64BIT
  // features. Float64x4([f64; 4]),
}
